...
<div class="conteiner-fluid container-max p-0 mx-auto">
    <div class="row justify-content-center px-0 pt-5 m-0">
        <div class="col-12 text-center">
            <h1>Inserisci un nuovo passepartout per il noleggio</h1>
        </div>
    </div>
</div>
  ...
  <div class="conteiner-fluid container-max p-0 mx-auto">
    <div class="mx-2">
      <form action="{{route('cardboardForRentals.store')}}" method="post" enctype="multipart/form-data">
        @csrf
        <section class="row pt-2 justify-content-center">
          <div class="col-12 col-md-2 mb-3">
            <label for="cardboard_id" class="form-label">cardboard_id</label>
                <select class="form-select rounded-0"  name="cardboard_id" value="{{old('cardboard_id')}}" id="cardboard_id" aria-label="Default select example" required>
          ...
          <div class="col-12 col-md-3 mb-3">
           ...
           <div class="col-12 col-md-2 mb-3">
            ...
            <input type="number" step="0.01" class="form-control rounded-0" max="30" name="pricePass" value="{{old('pricePass')}}" id="pricePass" required>
            @error('pricePass')
            <div class="alert alert-danger info-error">{{ $message }}</div>
            @enderror
          </div>
          <div class="col-12 col-md-2 mb-3">
            ...
            <input type="number" step="0.01" class="form-control rounded-0" max="30" name="priceFondo" value="{{old('priceFondo')}}" id="priceFondo" required>
            @error('priceFondo')
            <div class="alert alert-danger info-error">{{ $message }}</div>
            @enderror
          </div>
